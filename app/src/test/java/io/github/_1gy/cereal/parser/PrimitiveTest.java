...
public class PrimitiveTest {
    ...
    void beI8() {
        ...
        assertArrayEquals(new byte[] { 0x34 }, result.unwrap().rest().toByteArray());
        assertEquals((byte) 0x12, result.unwrap().value());

        // 入力バイト列に負数を含む場合
        var result2 = parser.parse(ByteSlice.of(new byte[] { (byte) 0x81, (byte) 0x82 }));
        assertEquals(true, result2.isOk());
        assertArrayEquals(new byte[] { (byte) 0x82 }, result2.unwrap().rest().toByteArray());
    }
    ...
    void beI16() {
        ...
        assertEquals(true, result.isOk());
        assertArrayEquals(new byte[] { 0x56 }, result.unwrap().rest().toByteArray());
        assertEquals((short) 0x1234, result.unwrap().value());

        // 入力バイト列に負数を含む場合
        var result2 = parser.parse(ByteSlice
                .of(new byte[] { (byte) 0x81, (byte) 0x82, (byte) 0x83 }));
        assertEquals(true, result2.isOk());
        assertArrayEquals(new byte[] { (byte) 0x83 }, result2.unwrap().rest().toByteArray());
        assertEquals((short) 0x8182, result2.unwrap().value());
    }
    ...
    void beI32() {
        ...
        assertEquals(true, result.isOk());
        assertArrayEquals(new byte[] { 0x01 }, result.unwrap().rest().toByteArray());
        assertEquals(0x12345678, result.unwrap().value());

        // 入力バイト列に負数を含む場合
        var result2 = parser
                .parse(ByteSlice.of(new byte[] { (byte) 0x81, (byte) 0x82, (byte) 0x83, (byte) 0x84, (byte) 0x85 }));
        assertEquals(true, result2.isOk());
        assertArrayEquals(new byte[] { (byte) 0x85 }, result2.unwrap().rest().toByteArray());
        assertEquals(0x81828384, result2.unwrap().value());
    }
    ...
    void beI64() {
        ...
        assertEquals(true, result.isOk());
        assertArrayEquals(new byte[] { 0x02 }, result.unwrap().rest().toByteArray());
        assertEquals(0x1234567801234567L, result.unwrap().value());
        ...
        assertEquals(true, result2.isOk());
        assertArrayEquals(new byte[] { (byte) 0x89 }, result2.unwrap().rest().toByteArray());
        assertEquals(0x8182838485868788L, result2.unwrap().value());
    }
    ...
    void test_t1ag() {
        ...
        assertEquals(true, result.isOk());
        assertArrayEquals("def".getBytes(), result.unwrap().rest().toByteArray());
        assertArrayEquals("abc".getBytes(), result.unwrap().value().toByteArray());
